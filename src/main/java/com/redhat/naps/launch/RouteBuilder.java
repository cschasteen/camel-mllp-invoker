...
public class RouteBuilder extends org.apache.camel.builder.RouteBuilder {
   ...
   static String malformHL7  
            = "MSH^~|\\&^HDRVTLS^589~KANSAS-CITY.MED.VA.GOV~DNS^R2-VITALS^200HD~HDR.MED.VA.GOV^<MESSAGE_TIMESTAMP>^^ORU~R01^<MESSAGE_CONTROL_ID>^P^2.4^^^AL^NE^USA" + '\n'
    ...
    public static String getHL7Message() {
        String tmpMessage = "";
        String stringMalform = "{{mllp.malform}}";
        if (stringMalform=="T") {
             tmpMessage = malformHL7.replaceFirst("<MESSAGE_TIMESTAMP>", timestampFormat.format(new Date()));
        }
        else{
             tmpMessage = hl7MessageTemplate.replaceFirst("<MESSAGE_TIMESTAMP>", timestampFormat.format(new Date()));
        }
